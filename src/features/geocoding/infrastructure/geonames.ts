...
import { Geocode, GeocodeSchema } from '@/features/geocoding/domain/value-objects/geocode';
...
export class GeonamesGeocodeRepository implements GeocodeRepository {
    ...
    public async fetchData(): Promise<void> {
        ...
        if (!fs.existsSync(txtFile)) {
            ...
            if (!resp.ok) {
                throw new Error(`Failed to download GeoNames dataset: ${resp.status} ${resp.statusText}`);
            }
    ...
    public async getGeocode(name: string): Promise<Result<Geocode | null, Error>> {
