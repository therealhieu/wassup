...
} from "../domain/entities/daily-weather-report";
import Image from "next/image";

const weatherIconUrl = "https://openweathermap.org/img/wn/";

const WeatherCodeInfoSchema = z.record(
	z.coerce.number(),
	z.object({
...
const WeatherIcon = ({ weatherCode, size, style }: WeatherIconProps) => {
	const iconPath =
		size == 1
			? `${WeatherCodeInfo[weatherCode].icon}.png`
			: `${WeatherCodeInfo[weatherCode].icon}@${size}x.png`;
			...
			<CardContent>
				...
				>
					<Typography variant="h4" sx={{ my: 2 }}>
						<div style={{ alignItems: "center" }}>
							<div style={{ position: "relative" }}>
								...
								>
									<Typography>
										{Math.round(report.temperatureMax)}°C |{" "}
										{Math.round(report.temperatureMin)}°C
									</Typography>
								</div>
							</div>
						</div>
					...
					>
						<span>
							precip:{" "}
							{Math.round(report.precipitationProbability)}%
						</span>
						<span>
							precip hrs: {Math.round(report.precipitationHours)}
						</span>
						...
						{reports.map((report) => (
							...
							>
								<Typography variant="body2">
									{report.date.toLocaleDateString(undefined, {
										weekday: "short",
									})}
								</Typography>
								<WeatherIcon
...
export const WeatherWidgetInnerPropsSchema = z.object({
	location: z.string(),
	reports: DailyWeatherReportListSchema,
});
export type WeatherWidgetInnerProps = z.infer<
	typeof WeatherWidgetInnerPropsSchema
>;

export const WeatherWidgetInner = ({
	location,
	reports,
